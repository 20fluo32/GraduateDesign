...
class Trainer(DDPTrainer):
    ...
    def __init__(
        ...
        self.writer = SummaryWriter(log_dir='./runs/tensorExperiment')  # 初始化TensorBoard
        if self.cfg.ddp:
    ...
    def train_epoch(self, epoch, train_loader):
        ...
        # 在每个epoch结束时也可以记录平均损失等
        avg_adv, avg_gram, avg_color, avg_content = self.loss_tracker.avg_loss_G()
        avg_adv_d = self.loss_tracker.avg_loss_D()
        self.writer.add_scalar('Loss/Adversarial', avg_adv, epoch)
        self.writer.add_scalar('Loss/Content', avg_content, epoch)
        self. writer.add_scalar('Loss/Gram', avg_gram, epoch)
        self.writer.add_scalar('Loss/Color', avg_color, epoch)
        self.writer.add_scalar('Loss/Discriminator', avg_adv_d, epoch)
